...
class DeleteCommentUseCase @Inject constructor(private val postRepository: PostRepository) {

    operator fun invoke(commentId: Long) : Flow<Result<Unit>> = flow{
        try {
            emit(Result.Loading())
            val response = postRepository.deleteComment(commentId)
