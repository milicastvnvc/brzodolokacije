...
class DeleteRateUseCase @Inject constructor(private val postRepository: PostRepository) {

    operator fun invoke(postId: Long) : Flow<Result<Double>> = flow{
        try {
            emit(Result.Loading())
            val response = postRepository.deleteRate(postId)
