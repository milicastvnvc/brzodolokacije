...
class SendVerificationUseCase @Inject constructor(private val authRepository: AuthRepository) {

    operator fun invoke(verificationCodeRequest : VerificationCodeRequest): Flow<Result<Unit>> = flow {
        try {
            emit(Result.Loading())

            val response = authRepository.sendVerificationToken(verificationCodeRequest)
